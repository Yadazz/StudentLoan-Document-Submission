...
async function fillPdf(userData, existingPdfBytes, fontBytes) {
  ...
  // คำนวณอายุ
  const age = calculateAge(userData.guardian_info.birth_date);  // คำนวณอายุ
  if (age === "Invalid Date") {
...
const nameWithoutPrefix = removePrefix(fullName);
  ...
  // แสดงเลขบัตรประชาชน
  const citizenId = userData.guardian_info.citizen_id || "-";
  ...
  if (userData.guardian_info.address_current) {
    firstPage.drawText(userData.guardian_info.address_current.house_no || "-", { x: 113.04, y: 615.4, size: 12, font: customFont });
    firstPage.drawText(userData.guardian_info.address_current.moo || "-", { x: 187.92, y: 615.4, size: 12, font: customFont });
    firstPage.drawText(userData.guardian_info.address_current.soi || "-", { x: 244.8, y: 615.4, size: 12, font: customFont });
    firstPage.drawText(userData.guardian_info.address_current.road || "-", { x: 378.72, y: 615.4, size: 12, font: customFont });
    firstPage.drawText(userData.guardian_info.address_current.sub_district || "-", { x: 72, y: 598, size: 12, font: customFont });
    firstPage.drawText(userData.guardian_info.address_current.district || "-", { x: 266.4, y: 598, size: 12, font: customFont });
    firstPage.drawText(userData.guardian_info.address_current.province || "-", { x: 430.56, y: 598, size: 12, font: customFont });
  }

  firstPage.drawText(userData.guardian_info.phone_number || "-", { x: 92.88, y: 580, size: 12, font: customFont });
  firstPage.drawText(userData.guardian_info.email || "-", { x: 317.52, y: 580, size: 12, font: customFont });
  firstPage.drawText(userData.name, { x: 69.12, y: 543, size: 12, font: customFont });
  firstPage.drawText(userData.guardian_info.name, { x: 241.2, y: 132, size: 12, font: customFont });

  return pdfDoc.save();
}

// ===== ฟังก์ชันรวมทั้งหมดสำหรับ หนังสือยินยอมมารดา =====
export const ConsentFrom_guardian = async () => {
  if (Platform.OS === "web") {
    Alert.alert("ไม่รองรับ Web", "การสร้าง PDF บน Web ยังไม่รองรับ");
    return;
  }
  ...
  try {
    ...
    const localFileName = "หนังสือยินยอมเปิดเผยข้อมูลผู้ปกครอง.pdf";
